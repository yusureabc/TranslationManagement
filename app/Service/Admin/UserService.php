...
class UserService extends BaseService
{
	...
	{
		return [$this->getAllPermissionList(),$this->getAllRoles(),$this->getMyPlatforms(),$this->getMyProducts()];
	}
	/**
	 * 获取所有权限并分组
	 * @author Sheldon
     ...
     * 获取所有的平台
     * @author Sheldon
     * @date   2017-05-04T13:23:46+0800
     * @return [type]                   [description]
     */
    public function getAllPlatforms()
    {
        return $this->platform->all(['id','name']);
    }

    public function getMyPlatforms ()
    {
        if (getUerId() == 1) {
            return $this->platform->all(['id','name']);
        }

        $platforms = $this->user->find(getUerId())->getPlatforms();
        $platformsIn = array_unique (array_pluck($platforms, 'id'));
        return $this->platform->findWhereIn('id', $platformsIn, ['id','name']);
    }
    ...
    {
        return $this->product->all(['id','name']);
    }
    ...
    public function getMyProducts ()
    {
        ...
        $products = $this->user->find(getUerId())->getProducts();
        $productsIn = array_unique (array_pluck($products, 'id'));
        return $this->product->findWhereIn('id', $productsIn, ['id','name']);
    }
	...
	public function storeUser($formData)
	{
		try {
			...
			if ($result) {
                ...
                // 更新用户平台产品关系
                if (isset($formData['platform']) && isset($formData['product'])) {
                    $result->products()->detach();
                    foreach ($formData['platform'] as $k => $v) {
                        foreach ($formData['product'] as $tk => $tv) {
                            $result->products()->attach($tv, ['platform_id' => $v]);
                        }
                    }
                }
			}
	...
	{
		return [$this->findUserById($id),$this->getAllPermissionList(),$this->getAllRoles(),$this->getAllPlatforms(),$this->getAllProducts()];
	}
	/**
	 * 根据ID获取权限数据
	 * @author Sheldon
	...
	{
		$role =  $this->user->with(['userPermissions','roles', 'platforms', 'products'])->find($id);
		if ($role) {
			return $role;
		}
		abort(404);
	...
	public function updateUser($attributes,$id)
	{
		...
		try {
			...
			if ($result) {
                ...
                // 更新用户平台产品关系
                if (isset($attributes['platform']) && isset($attributes['product'])) {
                    $result->products()->detach();
                    foreach ($attributes['platform'] as $k => $v) {
                        foreach ($attributes['product'] as $tk => $tv) {
                            $result->products()->attach($tv, ['platform_id' => $v]);
                        }
                    }
                } else {
                    $result->platforms()->sync([]);
                    $result->products()->sync([]);
                }
