...
Eof;
     ...
     * 平台列表
     * @author Sheldon
     * @date   2017-05-04T14:05:56+0800
     * @param  [type]                   $platforms [所有平台对象]
     * @param  [type]                   $userPlatforms [当前平台]
     * @return [type]                          [HTML]
     */
    public function platformList($platforms, $userPlatforms = [])
    {
        $html = '';
        if (!$platforms->isEmpty()) {
            foreach ($platforms as $platform) {
                $html .= '<label class="checkbox-inline"><div class="i-checks"><label> <input type="checkbox" name="platform[]" '.$this->checkPlatform($platform->id, $userPlatforms).' value="'.$platform->id.'"> '.$platform->name.'</label></div></label>';
            }
        }
        return $html;
    }
     ...
     * @param  array                    $userPlatforms [description]
     * @return [type]                              [description]
     */
    public function checkPlatform($platform_id, $userPlatforms = [])
    {
        $platforms = old('platform');
        if ($platforms) {
            return in_array($platform_id, $platforms) ? 'checked="checked"':'';
        }
        if ($userPlatforms) {
            if ($platforms) {
                if (in_array($platform_id,$userPlatforms) && in_array($platform_id,$platforms)) {
                    return 'checked="checked"';
                }
            }else{
                return in_array($platform_id,$userPlatforms) ? 'checked="checked"':'';
            }
            return '';
        }
        return '';
     ...
     * @author Sheldon
     * @date   2017-05-04T14:05:56+0800
     * @param  [type]                   $products [所有产品对象]
     * @param  [type]                   $userProducts [当前产品]
     * @return [type]                          [HTML]
     */
    public function productList($products, $userProducts = [])
    {
        $html = '';
        if (!$products->isEmpty()) {
            foreach ($products as $product) {
                $html .= '<label class="checkbox-inline"><div class="i-checks"><label> <input type="checkbox" name="product[]" '.$this->checkProduct($product->id, $userProducts).' value="'.$product->id.'"> '.$product->name.' </label></div></label>';
            }
        }
        return $html;
    }
     ...
     * @param  array                    $userProducts [description]
     * @return [type]                              [description]
     */
    public function checkProduct($product_id, $userProducts = [])
    {
        $products = old('product');
        if ($products) {
            return in_array($product_id, $products) ? 'checked="checked"':'';
        }
        if ($userProducts) {
            if ($products) {
                if (in_array($product_id,$userProducts) && in_array($product_id,$products)) {
                    return 'checked="checked"';
                }
            }else{
                return in_array($product_id,$userProducts) ? 'checked="checked"':'';
            }
            return '';
        }
        return '';
